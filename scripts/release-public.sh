...
# Release guard for the dsh-memory monorepo: validates both packages (build,
# clean tree, pack), then --publish pushes them to npm in dependency order
# (memory first, then tool-memory) after a confirmation prompt.
...
printf 'Publish memory + tool-memory at %s to %s? [y/N] ' "$(node -p "require('./packages/memory/package.json').version")" "$registry"
read -r answer
...
# tarball.
(cd packages/memory && pnpm publish --access public --registry "$registry" --no-git-checks)
(cd packages/tool-memory && pnpm publish --access public --registry "$registry" --no-git-checks)
